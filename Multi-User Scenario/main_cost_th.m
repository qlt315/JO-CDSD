...
B0_size_T = zeros(1,T);
% begin long term optimization in differernt cost_th
delay_T_cost_th = zeros(1,3);
delay_T_block_cost_th = zeros(1,3);
delay_T_instant_cost_th = zeros(1,3);
delay_uplink_cost_th = zeros(1,3);
delay_uplink_block_cost_th = zeros(1,3);
delay_uplink_instant_cost_th = zeros(1,3);

cach_cost_T_cost_th = zeros(1,3);
cach_cost_T_instant_cost_th = zeros(1,3);
cach_cost_T_block_cost_th = zeros(1,3);
cost_th_list = [1,2,3];
for i=1:3
    cost_th = cost_th_list(1,i);
for t = 1:T
    %% 当前states (所有方案统一使用的)
    ...
    delay_T_cost_th(1,i) = sum(delay_T(2,:)) / T;
    delay_T_block_cost_th(1,i) = sum(delay_T_block(2,:)) / T;
    delay_T_instant_cost_th(1,i) = sum(delay_T_instant(2,:)) / T;
    delay_uplink_cost_th(1,i) = sum(delay_uplink_T(2,:)) / T;
    delay_uplink_block_cost_th(1,i) = sum(delay_uplink_T_block(2,:)) / T;
    delay_uplink_instant_cost_th(1,i) = sum(delay_uplink_T_instant(2,:)) / T;

    cach_cost_T_cost_th(1,i) = sum(cach_cost_T(2,:)) / T;
    cach_cost_T_instant_cost_th(1,i) = sum(cach_cost_T_instant(2,:)) / T;
    cach_cost_T_block_cost_th(1,i) = sum(cach_cost_T_block(2,:)) / T;
...
% end
figure;
plot(delay_T_cost_th(1,:),'r-o','Linewidth',1);
hold on;
plot(delay_uplink_cost_th(1,:),'r--o','Linewidth',1);
hold on;
plot(delay_T_instant_cost_th(1,:),'b-*','Linewidth',1);
hold on;
plot(delay_uplink_instant_cost_th(1,:),'b--*','Linewidth',1);
hold on;
plot(delay_T_block_cost_th(1,:),'g-v','Linewidth',1);
hold on;
plot(delay_uplink_block_cost_th(1,:),'g--v','Linewidth',1);
grid on;
...
figure;
plot(cach_cost_T_cost_th(1,:)/3,'r-o','Linewidth',1);
hold on;
plot(cach_cost_T_instant_cost_th(1,:)/3,'b--*','Linewidth',1);
hold on;
plot([1,2,3],'k-','Linewidth',1);
hold on;
plot(cach_cost_T_block_cost_th(1,:)/3,'g--v','Linewidth',1);
grid on;
xlabel('cost threshold');
ylabel('averaged cost');
legend('cost', 'instant optimal', 'threshold', 'block descent');
